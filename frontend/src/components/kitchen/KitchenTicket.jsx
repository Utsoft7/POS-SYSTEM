export function printKitchenTicket(order, table) {
  try {
  const win = window.open("", "_blank", "width=260,height=600");
    const orderId = `#${Math.floor(new Date(order.orderDate).getTime())}`;
    const kot = order.kotNumber || `KOT-${Math.floor(Date.now() / 1000)}`;
    const tableNo = table?.tableNo ?? (order.table?.tableNo ?? "N/A");
    ...
    items.forEach((it, idx) => {
      ...
      itemsRows += `
        <tr>
          ...
          <td style="vertical-align:top;font-weight:700">${it.name.replace(/</g, '&lt;')}</td>
